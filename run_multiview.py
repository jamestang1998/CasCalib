...
for vid in terrace_array_names:
    ...
    (ankles, cam_matrix, normal, ankleWorld, focal, focal_batch, ransac_focal, datastore_filtered) = run_calibration_ransac(
         datastore, 'hyperparameter.json', img, 
    ...
    plane_data_2d, _ = geometry.camera_to_plane(data_2d, cam_matrix, plane_matrix, save_dict['ground_position'], save_dict["ground_normal"], img.shape[1], img.shape[0])
...
with open('outputs/plots/all_' + name + '/result_bundle_sync.csv','a') as file:
    ...
    writer1 = csv.writer(file)
    writer1.writerow([str(angle_diff1), str(focal_error1), str(results_position_diff1)])
    file.close
