    ...
    for itr in range(n_iter):
        ...
        for i in range(ransac_iter):
            ...
            for fr in frames:
                ...
                '''
                row_ind, col_ind = single_util.hungarian_assignment_one2one(ref_normalize, mov_transformed)
            ...
            R, t = single_geometry.rigid_transform_3D(np.transpose(X_mov), np.transpose(X_fix))
            ...
            for j in range(len(frame_ref)):
                ...
                error = single_util.chamfer_distance(frame_fix, frame_mov, metric='l2', direction='bi')
...
def icp_normalize(data_ref, data_sync, sync_time_dict, init_angle, init_ref_center, init_sync_center, reflect_true, n_iter = 10, save_dir = None, name = None, best_scale = 1.0):
    ...
    for track in data_interp_ref.keys():
        ...
        ref_vel = torch.squeeze(single_util.central_diff(torch.unsqueeze(torch.transpose(torch.tensor(ref_points), 0 , 1), dim = 1).double(), time = 1, dilation = dilation))
    ...
    for track in data_interp_sync.keys():
        ...
        sync_vel = torch.squeeze(single_util.central_diff(torch.unsqueeze(torch.transpose(torch.tensor(sync_points), 0 , 1), dim = 1).double(), time = best_scale, dilation = dilation))
    ...
    for itr in range(n_iter):
        ...
        #T, inliers = cv2.estimateAffinePartial2D(X_fix, X_mov)
        R, t = single_geometry.rigid_transform_3D(np.transpose(X_mov), np.transpose(X_fix))
        #R, t = single_geometry.rigid_transform_3D(np.transpose(X_mov_vel), np.transpose(X_fix_vel))
    ...
    for t in track_dict.keys():
        ...
        for i in range(len(track_dict[t])):
            ...
            if i == len(track_dict[t]) - 1:
                ...
                y_new = single_util.interpolate(points, time, grid)
