import sys
sys.path.append('/local/tangytob/Summer2023/camera_calibration_synchronization/') 
import numpy as np
import math
...
def evaluate_relative(cam_intrinsics_array, cam_axis, cam_position, intrinsic_array_tsai, cam_axis_tsai, cam_position_tsai, output_path, data = 'result'):
    ...
    for c in range(1, len(cam_axis)):
        ...
        relative_angle = np.rad2deg(np.linalg.inv(rotation_pred) @ rotation_tsai)
        relative_angle1 = np.rad2deg(np.linalg.inv(rotation_tsai) @ rotation_pred)
        print(relative_angle, relative_angle1, " ANGLE")
