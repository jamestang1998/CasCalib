...
def calibration_focalpoint_lstq_failure_single(num_points, hv, av, hu, au, h, t1, t2, focal_predicted = None, upper_bound = np.inf, h_conf = [], al_conf = [], ar_conf = []):
    ...
    C1 = np.zeros([2*num_points,3 + num_points], dtype = float)
    ...
    for col in range(0, 2*num_points):
        if col % 2 == 0:
            C1[col][1] = -1
            C1[col][2] = av[int(col/2)]
            C1[col][3 + int(col/2)] = av[int(col/2)] - hv[int(col/2)]
            C1[col + 1][0] = 1
            C1[col + 1][2] = -au[int(col/2)]
            C1[col + 1][3 + int(col/2)] = hu[int(col/2)] - au[int(col/2)]
    try:
        ...
        if len(al_conf) == 0:
            ...
            for i in range(num_points):
                ...
                C1[2*i, :] = avg_conf*C1[2*i, :]
                C1[2*i + 1, :] = avg_conf*C1[2*i + 1, :]

            U, S, Vh = np.linalg.svd(C)
            L = (Vh[-1, :])

            U1, S1, Vh = np.linalg.svd(C1)
            L1 = (Vh[-1, :])
    except:
        return None, None, None, None, None, None
    if focal_predicted is None:
        if len(al_conf) == 0:
            focal_predicted = compute_focal_failure_single_lstq(au, av, hu, hv, L, L1, t1, t2, upper_bound = upper_bound)
        else:
            focal_predicted = compute_focal_failure_single_weighted_lstq(au, av, hu, hv, ar_conf, al_conf, h_conf, L, L1, t1, t2, upper_bound = upper_bound)
...
def compute_focal_failure_single_lstq(au, av, hu, hv, L, L1, t1, t2, upper_bound = np.inf):
    ...
    for comb in comb_array:   
        ...
        eq2_a1 = -(L1[0] - L1[2]*t1)*(L1[3 + comb[0]]*(hu[comb[0]] - t1) - L1[3 + comb[1]]*(hu[comb[1]] - t1)) 
        eq2_a2 = -(L1[1] - L1[2]*t2)*(L1[3 + comb[0]]*(hv[comb[0]] - t2) - L1[3 + comb[1]]*(hv[comb[1]] - t2))
        eq2_b = (L1[2]*(L1[3 + comb[0]] - L1[3 + comb[1]]))
        
        #A = np.array([[eq1_a1, eq1_a2], [eq2_a1, eq2_a2]])
        #b = np.array([eq1_b, eq2_b])
        if eq1_b/(eq1_a1 + eq1_a2) < 0 or eq2_b/(eq2_a1 + eq2_a2) < 0:
            continue
        A.append([eq1_a1 + eq1_a2])
        A.append([eq2_a1 + eq2_a2])
        b.append(eq1_b)
        b.append(eq2_b)
...
def compute_focal_failure_single_weighted_lstq(au, av, hu, hv, ar_conf, al_conf, h_conf, L, L1, t1, t2, upper_bound = np.inf):
    ...
    for comb in comb_array:   
        ...
        average_conf_ankle = (ar_conf[comb[0]] + al_conf[comb[0]] + ar_conf[comb[1]] + al_conf[comb[1]])/4.0

        average_conf_head = (h_conf[comb[0]] + h_conf[comb[1]])/2.0
        ...
        if eq1_b/(eq1_a1 + eq1_a2) < 0 or eq2_b/(eq2_a1 + eq2_a2) < 0:
            continue
        A.append([average_conf_ankle*eq1_a1 + average_conf_ankle*eq1_a2])
        A.append([average_conf_head*eq2_a1 + average_conf_head*eq2_a2])
        b.append(average_conf_ankle*eq1_b)
        b.append(average_conf_head*eq2_b)
